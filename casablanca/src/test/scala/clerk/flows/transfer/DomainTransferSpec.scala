...
class DomainTransferSpec extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  "DomainTransferHandlerFactory " should " support all aspects of status sequentially " in {
    ...
    val task = domainTransferHandlerFactory.createInitTask(wfm.statusQManager.taskContext, "domainName", "aspirantId", "ownerId")
    ...
    initialiseTransferHndlr.handle(wfm.statusQManager.taskContext, task) match {
